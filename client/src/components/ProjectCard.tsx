    ...
    <div key={gen.id} className="mb-4 break-inside-avoid">
        ...
        {/*preview */}
           <div className={`${gen?.aspectRatio === '9:16' ? 'aspect-9/16' : 'aspect-video'} relative overflow-hidden`}>
            ...
            {(!gen?.generatedImage && !gen?.generatedVideo) && ( <div className="absolute inset-0 w-full h-full flex flex-col items-center justify-center bg-black/20"> <Loader2Icon className="size-7 animate-spin" /></div>)}
        ...
        {/* product name, date, aspect ration */}
           <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                    <h3 className="mb-1 truncate text-lg font-medium">{gen.productName}</h3>
                    <p className="whitespace-nowrap text-sm text-gray-400">Created: {new Date(gen.createdAt).toLocaleString()}</p>
                    {gen.updatedAt && ( <p className="mt-1 whitespace-nowrap text-xs text-gray-500">Updated: {new Date(gen.updatedAt).toLocaleString()}</p>)}
                </div>
           ...
           {gen.productDescription && ( 
                ...
                <div className="text-sm text-gray-300 bg-white/3 p-2 rounded-md break-words">{gen.productDescription}</div>
            </div>)}
           {/* user prompt */}
           {gen.userPrompt && ( 
                <div className="mt-3">
